...
final class TheHouseSaysWhatItHasChangedTest extends TestCase
{
    ...
    /**
     * THE TABLE RENDERS AS A TABLE, which the unit tests above could never have caught.
     *
     * 🚨 The first version built an array of CELLS per row and handed it to `table()`, which takes
     * RENDERED rows and `implode`s them — so every diverged file would have printed the literal word
     * «Array». phpstan named it at the argument; no test here would have, because these test the data
     * and the suite never rendered this table with a diverged row in it (greenhouse decisions/0293).
     */
    public function testTheDivergedFilesRenderAsRowsAndNotAsTheWordArray(): void
    {
        $tree = $this->house();
        file_put_contents($tree . '/public/index.php', "<?php // mine now\n");
        unlink($tree . '/bin/coa');

        $html = (new \Milpa\Admin\Rendering\AdminHtmlRenderer(
            new \Milpa\Live\Security\SignedXhtmlStateTransferCodec(
                new \Milpa\Live\Transport\XhtmlStateTransferCodec(),
                new \Milpa\Live\Security\HmacStateSigner('divergence-secret-0123456789'),
                null,
            ),
            new \Milpa\Admin\I18n\Catalog(),
            \Milpa\Admin\AdminSettings::fromConfig(null),
        ))->render(
            new \Milpa\Admin\Components\HouseComponent(new \Milpa\Admin\Data\HouseSource(
                new \Milpa\Container\DIContainer(),
                \Milpa\Admin\AdminSettings::fromConfig(null),
                new \Milpa\Admin\Data\PluginsSource(new \Milpa\Container\DIContainer()),
                new \Milpa\Admin\Data\RoutesSource(new \Milpa\Container\DIContainer()),
            )),
            new \Milpa\Live\ValueObjects\RenderRequest(
                context: new \Milpa\Live\ValueObjects\ComponentContext(componentId: 'h1'),
                props: [],
                state: new \Milpa\Live\ValueObjects\StateSnapshot('h1', \Milpa\Admin\Components\HouseComponent::NAME, '1', [
                    'divergence' => FrameworkDivergence::summary($tree),
                    'divergenceRows' => FrameworkDivergence::rows($tree),
                ], ['title' => 'The house']),
                target: \Milpa\Live\ValueObjects\RenderTarget::HTML,
            ),
        )->output;

        self::assertStringNotContainsString('>Array<', $html, 'cells handed to table() would have imploded as the word Array');
        self::assertStringContainsString('<td><code class="admin-house__path">public/index.php</code></td>', $html);
        self::assertStringContainsString('Customized', $html);
        self::assertStringContainsString('Deleted', $html);
        self::assertStringNotContainsString('config/app.php', $html, 'an untouched file is counted, never listed');
    }
}
